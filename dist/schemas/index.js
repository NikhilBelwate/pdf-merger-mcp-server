...
export const UploadPdfsInputSchema = z
    ...
    file_paths: z
        ...
        .describe("Absolute file paths to PDF files on the local filesystem (e.g., [\"/home/user/doc1.pdf\", \"/home/user/doc2.pdf\"])"),
    session_id: z
        .string()
        .uuid("session_id must be a valid UUID")
        .optional()
        .describe("Optional existing session ID to append files to. Omit to create a new session."),
})
    .strict();
/* ------------------------------------------------------------------ */
...
export const MergePdfsInputSchema = z
    ...
    file_order: z
        ...
        .describe("Ordered array of file IDs specifying the merge sequence. The first ID becomes the first pages of the output."),
})
    .strict();
/* ------------------------------------------------------------------ */
...
export const GetDownloadUrlInputSchema = z
    ...
    token: z
        ...
        .describe("One-time download token returned by the merge operation. Each token can only be used once."),
})
    .strict();
